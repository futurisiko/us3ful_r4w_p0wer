...
$shares | ForEach-Object { $fullpath = $rootPath + $_ ; Get-ChildItem "$fullpath" } | Where-Object { Get-Acl $_.FullName | Select-Object -ExpandProperty Access | Where-Object { $_.IdentityReference -match 'Users' } } | ForEach-Object { Write-Host $_.FullName -ForegroundColor green; Get-Acl $_.FullName | Select-Object -ExpandProperty Access | Where-Object { $_.IdentityReference -match 'Users' } | Select-Object FileSystemRights, IdentityReference} | FL 2>$null
